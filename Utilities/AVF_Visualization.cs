    ...
    public static class Visualization
    {
        ...
        public static void ApplyVisualization(Document doc, View view, Dictionary<Element, List<Reference>> elementGrainData, AnalysisDisplayStyle analysisDisplayStyle)
        {
            ...
            foreach (var kvp in elementGrainData)
            {
                ...
                {
                    SelectedElementId elementId = reference.SelectedElementId;
...
/*
 //this is the previous that works perfect
...
//this isthe other VISUALIZATION CODE 
            ...
            foreach (var kvp in elementGrainData)
            {
                foreach (Reference reference in kvp.Value)
                {
                    ...
                    if (face != null)
                    {
                        ...
                        // Add debug information to check the counts before applying the color
                        Debug.WriteLine($"Element: {element.Id}, Face: {face.Id}");
                        Debug.WriteLine($"Number of UV points: {pts.Count}, Number of values: {valuesAtPoints.Count}");

                        // Check for a mismatch in counts and throw an exception if there is a mismatch
                        if (pts.Count != valuesAtPoints.Count)
                        {
                            throw new InvalidOperationException("The number of UV points does not match the number of ValueAtPoint entries.");
                        }
